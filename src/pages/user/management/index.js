// ** React Imports
import { useState, useEffect } from 'react'

// ** MUI Imports
import Card from '@mui/material/Card'
import CardHeader from '@mui/material/CardHeader'
import { DataGrid } from '@mui/x-data-grid'
import Grid from '@mui/material/Grid'
import Button from '@mui/material/Button'
import axios from 'axios'
import {
  GridToolbarColumnsButton,
  GridToolbarContainer,
  GridToolbarExport,
  GridToolbarFilterButton,
  GridToolbarQuickFilter
} from '@mui/x-data-grid'
import Typography from '@mui/material/Typography'
import Tooltip from '@mui/material/Tooltip'

// ** Views
import DialogAddUser from 'src/views/pages/user/management/DialogAddUser'
import DialogAddGuardian from 'src/views/pages/guardian/management/DialogAddGuardian'
import DialogViewUser from 'src/views/pages/user/management/DialogViewUser'
import DialogViewGuardian from 'src/views/pages/guardian/management/DialogViewGuardian'
import DialogDeleteUser from 'src/views/pages/user/management/DialogDeleteUser'
import DialogDeleteGuardian from 'src/views/pages/guardian/management/DialogDeleteGuardian'
import DialogViewVehicles from 'src/views/pages/user/vehicles/management/DialogViewVehicles'
import DialogViewGaurdianVehicles from 'src/views/pages/guardian/vehicles/management/DialogViewVehicles'

function CustomToolbar(props) {
  const { setUserRows } = props

  // Refresh list of users
  const fetchUsers = () => {
    axios
      .get('/api/user')
      .then(response => {
        setUserRows(response.data)
      })
      .catch(error => console.error('Error fetching data', error))
  }

  return (
    <GridToolbarContainer style={{ display: 'flex', justifyContent: 'space-between' }}>
      <div>
        <DialogAddUser refreshData={fetchUsers} />
        <GridToolbarFilterButton style={{ marginRight: '8px', marginBottom: '8px' }} />
      </div>
      <div>
        <Button
          size='small'
          variant='outlined'
          style={{ marginLeft: '8px', marginRight: '8px', marginBottom: '8px' }}
          onClick={() => fetchUsers()}
        >
          Refresh
        </Button>
        <GridToolbarQuickFilter style={{ marginBottom: '8px' }} />
      </div>
    </GridToolbarContainer>
  )
}

function CustomGuardianToolbar(props) {
  const { userRows, setGuardianRows } = props

  // Refresh list of users
  const fetchGuardians = () => {
    axios
      .get('/api/guardian')
      .then(response => {
        setGuardianRows(response.data)
      })
      .catch(error => console.error('Error fetching data', error))
  }

  return (
    <GridToolbarContainer style={{ display: 'flex', justifyContent: 'space-between' }}>
      <div>
        <DialogAddGuardian users={userRows} refreshData={fetchGuardians} />
        <GridToolbarFilterButton style={{ marginRight: '8px', marginBottom: '8px' }} />
      </div>
      <div>
        <Button
          size='small'
          variant='outlined'
          style={{ marginLeft: '8px', marginRight: '8px', marginBottom: '8px' }}
          onClick={() => fetchGuardians()}
        >
          Refresh
        </Button>
        <GridToolbarQuickFilter style={{ marginBottom: '8px' }} />
      </div>
    </GridToolbarContainer>
  )
}

const UserManagement = () => {
  // ** States
  const [userPaginationModel, setUserPaginationModel] = useState({ page: 0, pageSize: 10 })
  const [guardianPaginationModel, setGuardianPaginationModel] = useState({ page: 0, pageSize: 10 })
  const [userRows, setUserRows] = useState([])
  const [guardianRows, setGuardianRows] = useState([])

  // Refresh list of users
  const fetchUsers = () => {
    axios
      .get('/api/user')
      .then(response => {
        setUserRows(response.data)
      })
      .catch(error => console.error('Error fetching data', error))
  }

  const fetchGuardians = () => {
    axios
      .get('/api/guardian')
      .then(response => {
        setGuardianRows(response.data)
      })
      .catch(error => console.error('Error fetching data', error))
  }

  // Fetch data on component mount
  useEffect(() => {
    fetchUsers()
    fetchGuardians()
  }, [])

  const userColumn = [
    {
      flex: 0.1,
      minWidth: 250,
      field: 'fullName',
      headerName: 'Full Name',
      valueGetter: params => params.row.first_name + ' ' + params.row.last_name,
      renderCell: params => (
        <Tooltip title={params.row.first_name + ' ' + params.row.last_name}>
          <Typography variant='body2' sx={{ color: 'text.primary' }}>
            {params.row.first_name + ' ' + params.row.last_name}
          </Typography>
        </Tooltip>
      )
    },
    {
      flex: 0.1,
      minWidth: 250,
      field: 'load_balance',
      headerName: 'Load Balance',
      valueGetter: params => params.row.load_balance,
      renderCell: params => (
        <Tooltip title={params.row.load_balance}>
          <Typography variant='body2' sx={{ color: 'text.primary' }}>
            {params.row.load_balance}
          </Typography>
        </Tooltip>
      )
    },
    {
      flex: 0.1,
      minWidth: 50,
      field: 'status',
      headerName: 'Status',
      valueGetter: params => params.row.status,
      renderCell: params => (
        <Tooltip title={params.row.status}>
          <Typography variant='body2' sx={{ color: 'text.primary' }}>
            {params.row.status}
          </Typography>
        </Tooltip>
      )
    },
    {
      flex: 0.2,
      minWidth: 100,
      field: 'action',
      headerName: 'Actions',
      renderCell: params => {
        return (
          <>
            <DialogViewUser user={params.row} users={userRows} refreshData={fetchUsers} />
            <DialogViewVehicles user_id={params.row.id} refreshData={fetchUsers} />
            <DialogDeleteUser user_id={params.row.id} refreshData={fetchUsers} />
          </>
        )
      }
    }
  ]

  const guardianColumn = [
    {
      flex: 0.1,
      minWidth: 100,
      field: 'fullName',
      headerName: 'Full Name',
      valueGetter: params => params.row.first_name + ' ' + params.row.last_name,
      renderCell: params => (
        <Tooltip title={params.row.first_name + ' ' + params.row.last_name}>
          <Typography variant='body2' sx={{ color: 'text.primary' }}>
            {params.row.first_name + ' ' + params.row.last_name}
          </Typography>
        </Tooltip>
      )
    },
    {
      flex: 0.1,
      minWidth: 100,
      field: 'student',
      headerName: 'Student Name',
      valueGetter: params => params.row.first_name + ' ' + params.row.last_name,
      renderCell: params => (
        <Tooltip title={params.row.first_name + ' ' + params.row.last_name}>
          <Typography variant='body2' sx={{ color: 'text.primary' }}>
            {params.row.first_name + ' ' + params.row.last_name}
          </Typography>
        </Tooltip>
      )
    },
    {
      flex: 0.1,
      minWidth: 100,
      field: 'load_balance',
      headerName: 'Load Balance',
      valueGetter: params => params.row.load_balance,
      renderCell: params => (
        <Tooltip title={params.row.load_balance}>
          <Typography variant='body2' sx={{ color: 'text.primary' }}>
            {params.row.load_balance}
          </Typography>
        </Tooltip>
      )
    },
    {
      flex: 0.1,
      minWidth: 50,
      field: 'status',
      headerName: 'Status',
      valueGetter: params => params.row.status,
      renderCell: params => (
        <Tooltip title={params.row.status}>
          <Typography variant='body2' sx={{ color: 'text.primary' }}>
            {params.row.status}
          </Typography>
        </Tooltip>
      )
    },
    {
      flex: 0.2,
      minWidth: 500,
      field: 'action',
      headerName: 'Actions',
      renderCell: params => {
        return (
          <>
            <DialogViewGuardian users={userRows} guardian={params.row} refreshData={fetchGuardians} />
            <DialogViewGaurdianVehicles guardian_id={params.row.id} refreshData={fetchGuardians} />
            <DialogDeleteGuardian guardian_id={params.row.id} refreshData={fetchGuardians} />
          </>
        )
      }
    }
  ]

  return (
    <Grid container spacing={8}>
      <Grid item sm={12} xs={12} sx={{ width: '100%' }}>
        <Card>
          <CardHeader title='Users' />
          <DataGrid
            autoHeight
            columns={userColumn}
            rows={userRows}
            getRowId={row => row.id}
            pageSizeOptions={[10, 25, 50, 100]}
            paginationModel={userPaginationModel}
            slots={{ toolbar: CustomToolbar }}
            onPaginationModelChange={setUserPaginationModel}
            slotProps={{
              baseButton: {
                variant: 'outlined'
              },
              toolbar: {
                setUserRows
              }
            }}
          />
        </Card>
      </Grid>
      <Grid item sm={12} xs={12} sx={{ width: '100%' }}>
        <Card>
          <CardHeader title='Guardians' />
          <DataGrid
            autoHeight
            columns={guardianColumn}
            rows={guardianRows}
            getRowId={row => row.id}
            pageSizeOptions={[10, 25, 50, 100]}
            paginationModel={guardianPaginationModel}
            slots={{ toolbar: CustomGuardianToolbar }}
            onPaginationModelChange={setGuardianPaginationModel}
            slotProps={{
              baseButton: {
                variant: 'outlined'
              },
              toolbar: {
                userRows,
                setGuardianRows
              }
            }}
          />
        </Card>
      </Grid>
    </Grid>
  )
}

UserManagement.acl = {
  action: 'read',
  subject: 'user-management'
}

export default UserManagement
