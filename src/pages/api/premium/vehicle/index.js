...
const addVehicle = async data => {
  const { premium_id, maker, model, color, image, rfid, plate_number, cr_number, or_number, registration_expiration } =
    data
  ...
  try {
    const [vehicleResult] = await db.query(
      'INSERT INTO vehicles (premium_id, maker, model, color, image, plate_number, cr_number, or_number, registration_expiration) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [
        premium_id,
        maker,
        model,
    ...
    const [existingVehicles] = await db.query('SELECT COUNT(*) as count FROM vehicles WHERE premium_id = ?', [
      premium_id
    ])
