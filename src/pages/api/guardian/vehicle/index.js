...
const checkRFIDUnique = async (rfid, vehicleID = null, currentUserRfid = null) => {
  let query = `SELECT COUNT(*) as count FROM rfids WHERE value = ?`
  const params = [rfid]

  if (vehicleID && currentUserRfid !== rfid) {
...
const addVehicle = async data => {
  const { guardian_id, maker, model, color, image, rfid, plate_number, cr_number, or_number, registration_expiration } =
    data
  ...
  try {
    const [vehicleResult] = await db.query(
      'INSERT INTO vehicles (guardian_id, maker, model, color, image, plate_number, cr_number, or_number, registration_expiration) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [
        guardian_id,
        maker,
        model,
    ...
    const [existingVehicles] = await db.query('SELECT COUNT(*) as count FROM vehicles WHERE guardian_id = ?', [
      guardian_id
    ])

    if (existingVehicles[0].count > 0) {
      await db.query('UPDATE guardians SET status = ? WHERE id = ?', ['Inactive', guardian_id])
    }
...
const editVehicle = async data => {
  ...
  const currentUserRfid = currentVehicle[0].rfid
  ...
  const isRfidUnique = await checkRFIDUnique(rfid, vehicle_id, currentUserRfid)
