...
const ChangePasswordCard = ({ premium_id }) => {
  // ** States
  ...
  const onPasswordFormSubmit = async data => {
    try {
      const response = await axios.put(`/api/user/premium/settings/${premium_id}`, {
        data: {
      ...
      const errorMessage =
        ...
        error.response?.data?.error || // Another potential API error field
