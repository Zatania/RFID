...
const DialogViewVehicles = ({ guardian_id, refreshData }) => {
  ...
  useEffect(() => {
    if (show) {
      fetchVehicles(guardian_id)
    }
  }, [show, guardian_id])

  const fetchVehicles = async guardian_id => {
    try {
      const response = await axios.get(`/api/guardian/${guardian_id}`)
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      ...
      >
          ...
          {vehicles.map((vehicle, index) => (
            <Grid container spacing={6} key={vehicle.id}>
              ...
              <Grid item sm={12} xs={12}>
                ...
                >
                  <DialogViewVehicle vehicle={vehicle} refreshData={() => fetchVehicles(guardian_id)} />
                  <DialogDeleteVehicle vehicle_id={vehicle.id} refreshData={() => fetchVehicles(guardian_id)} />
                </Grid>
              </Grid>
            </Grid>
          ))}
        ...
        >
          <DialogAddVehicle guardian_id={guardian_id} refreshData={() => fetchVehicles(guardian_id)} />
